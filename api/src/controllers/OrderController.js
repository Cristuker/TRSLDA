	...
	async update(req, res) {
		try {
			...
			if (!response) {
				return res
					.status(400)
					.json({ message: 'This order not exists' });
			}
			...
			if (req.body.start_date) {
				...
				if (!iCanGetOrder) {
					return res
						.status(400)
						.json({ message: 'Start hour is invalid' });
				}
			}
			...
			if (req.body.end_date) {
				if (
					...
					return res
						.status(400)
						.json({ message: 'End date is before start date' });
				}
			}
			...
			if (req.body.canceled_at) {
				if (
					...
					return res.status(400).json({
						message: 'Canceled date is before start date',
					});
				}
			}

			const updateResponse = await Order.update(req.body, {
				where: { id },
			});
